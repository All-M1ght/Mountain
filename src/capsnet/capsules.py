...
class PrimaryCapsules(nn.Module):
    def __init__(self, in_channels, out_channels, dim_caps,
        ...
        # print("_caps_channel:" + str(self._caps_channel))
...
class RoutingCapsules(nn.Module):
    def __init__(self, in_dim, in_caps, num_caps, dim_caps, num_routing, device: torch.device):
        ...
        Initialize the layer.
        Args:
            in_dim: 		Dimensionality (i.e. length) of each capsule vector.输入胶囊的维度
            in_caps: 		Number of input capsules if digits layer. 输入胶囊数
            num_caps: 		Number of capsules in the capsule layer 下一层胶囊数
            dim_caps: 		Dimensionality, i.e. length, of the output capsule vector.输出胶囊的维度
            num_routing:	Number of iterations during routing algorithm 路由聚类迭代次数
        """
